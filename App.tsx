...
function App() {
  ...
  useEffect(() => {
    const checkKey = async () => {
      ...
      if (win.aistudio && win.aistudio.hasSelectedApiKey) {
        ...
        // Fallback for local development or environments without the wrapper
        setHasApiKey(true);
      }
    };
    checkKey();
  ...
  if (!hasApiKey) {
     return (
       <div className="w-full h-screen bg-gray-900 flex flex-col items-center justify-center text-white p-4">
          <div className="bg-gray-800 p-8 rounded-2xl shadow-2xl max-w-md text-center border border-gray-700">
            ...
            <p className="text-gray-300 mb-8">
              To generate Lego sets from images, please select your Google Cloud API Key.
            </p>
            <button 
              onClick={handleSelectKey}
              className="w-full bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white font-bold py-4 px-6 rounded-xl transition-all transform hover:scale-105 shadow-lg"
            >
              Select API Key
            </button>
            <div className="mt-6 text-xs text-gray-500">
               Must use a paid project. <a href="https://ai.google.dev/gemini-api/docs/billing" target="_blank" className="underline hover:text-gray-300">Billing Information</a>
            </div>
          </div>
       </div>
     );
